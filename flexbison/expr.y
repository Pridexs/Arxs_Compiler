...
%%
Inicio: Programa { geraArquivoFinal(); }
      ;
...
ListaFuncoes: ListaFuncoes Funcao { insereDelimitadorFuncao(); }
...
DeclParametros: DeclParametros TVIRG Tipo TID { insereParametro(&($$.listaParametros), $3.tipo, $4.id); }
...
Declaracao: Tipo ListaId TPVIRG { insereTabelaSimbolos( (&$2.listaIDs), $1.tipo );}
          ;
...
ListaId: ListaId TVIRG TID { insereID( &($$.listaIDs), $3.id); }
...
Retorno: TRETURN ExpressaoAritmetica TPVIRG { geraRetorno($2.tipo); }
       ;

CmdSe: TIF TAPAR ExpressaoLogica TFPAR NL Bloco { corrigir(&$3.listav, $5.label); corrigir(&$3.listaf, novoLabel()); }
...
N: %empty { inicializaListaVF(&$$.listav, &$$.listaf); gerarGOTO(); }

CmdEnquanto: TWHILE NL TAPAR ExpressaoLogica TFPAR NL Bloco
             {
...
CmdAtrib: TID TATRIB ExpressaoAritmetica TPVIRG { geraAtribuicao( $1.id, buscaTipo($1.id), $3.tipo); }
        | TID TATRIB TLITERAL TPVIRG { empilhaString($3.literal); geraAtribuicao( $1.id, buscaTipo($1.id), TIPO_STRING); }
        ;

CmdEscrita: TPRINT TAPAR M ExpressaoAritmetica TFPAR TPVIRG { if ($4.tipo == TIPO_INT) invokeStatic(TIPO_INT); else invokeStatic(TIPO_STRING); }
...
ChamadaFuncao: TID TAPAR ListaParametros TFPAR TPVIRG { geraChamadaFuncaoComPar($1.id, $3.nParametros); geraChamadaFuncaoComPar($1.id, $3.nParametros); }
             | TID TAPAR TFPAR TPVIRG { geraChamadaFuncao($1.id); }
             ;

ChamadaFuncaoSemPVIRG: TID TAPAR ListaParametros TFPAR { $$.tipo = buscaTipoFuncao($1.id); geraChamadaFuncaoComPar($1.id, $3.nParametros); }
                     | TID TAPAR TFPAR { $$.tipo = buscaTipoFuncao($1.id); geraChamadaFuncao($1.id); }
                     ;

ListaParametros: ListaParametros TVIRG ExpressaoAritmetica { $$.nParametros += 1; }
               | ExpressaoAritmetica { $$.nParametros = 1; }
               ;

ExpressaoAritmetica: ExpressaoAritmetica TADD TermoAritmetico { geraOperacao(OP_ADD); }
                   | ExpressaoAritmetica TSUB TermoAritmetico { geraOperacao(OP_SUB); }
                   | TermoAritmetico { $$.tipo == $1.tipo; }
                   ;

TermoAritmetico: TermoAritmetico TMUL FatorAritmetico { if ($1.tipo == $3.tipo) geraOperacao(OP_MUL); else error(); }
               | TermoAritmetico TDIV FatorAritmetico { if ($1.tipo == $3.tipo) geraOperacao(OP_DIV); else error(); }
               | FatorAritmetico { $$.tipo = $1.tipo; }
               ;

FatorAritmetico: TNUM { empilhaInteiro($1.constante); $$.tipo = TIPO_INT; }
...
ExpressaoLogica: ExpressaoLogica TAND NL FatorLogico 
                 {
...
NL: %empty { $$.label = novoLabel(); }
  ;

FatorLogico: TNOT FatorLogico { transfereListaVF(&$$.listav, &$$.listaf, &$2.listav, &$2.listaf); }
...
ExpressaoRelacional: ExpressaoAritmetica TMAIOR ExpressaoAritmetica { inicializaListaVF(&$$.listav, &$$.listaf); geraComparacao(CMP_GTHAN); gerarGOTO(); }
